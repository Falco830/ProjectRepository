			...
			if(isOperator(equationElements[i].charAt(0))) {
				...
				if(operatorStack.isEmpty()) {
					...
					if(equationElements[i].charAt(0) == '-' && !isOperator(equationElements[i+1].charAt(0))) {
						...
						//operatorStack.push('+');
						while(!operatorStack.isEmpty() && priorityCheck(operatorStack.peek()) > priorityCheck('+') ) {	//Scala 						
							System.out.println(operatorStack.peek());
							newEquation += operatorStack.pop() + ", ";						
						}
						/*
						if(!operatorStack.isEmpty() && priorityCheck(operatorStack.peek()) == priorityCheck('+')){
							operatorStack.push('+');
								break;
						}*/
						newEquation += equationElements[i]+equationElements[i+1] + ", ";
			...
			if(equation.charAt(i) == '-') {
				...
				if(i>0){
					if(isOperator(Character.toString(equation.charAt(i-1))) && !isOperator(Character.toString(equation.charAt(i+1)))){
						if(equation.charAt(i-1) == ')' && (i != equation.length()-2)) {
							//newEquation += "+";
						}
						...
						for(int j = i+1; j < equation.length();j++) {
							if(isOperator(Character.toString(equation.charAt(j)))){
								...
								System.out.println("a1 " + newEquation);
								i = j-1;															//did not exist
								break;
							}else if(j == equation.length()-1) {
								newEquation += /*"(0" + */equation.substring(i, j+1)/* + ")"*/;			//was at i-1
								System.out.println("a2 " +  newEquation);
								i = j;															//did not exist
								break;
							}
